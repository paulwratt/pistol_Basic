...
2050   m(X,Y,1)=64 : m(X,Y,2)=255 : m(X,Y,3)=64 : m(X,Y,4)=A : if B>251 then gosub 2111
2082  next X
2091 next Y
2100 rem * generate special resources *
2111 for Y=1 to H
2122  for X=p to mapx
2130   R=rnd(255) : G=rnd(255) : B=rnd(255) : A=rnd(255)
2150   if B<60 then m(X,Y,1)=R : m(X,Y,2)=G : m(X,Y,3)=B : m(X,Y,4)=A
2182  next X
2191 next Y
2199 goto 3600 : ' map gen done; jump to next part
